...
int DoVolPanning256(short whichChannel, MADChannel *ch, MADDriverRec *intDriver, bool Interpol)	// MAX = 256
{
	...
	bool previewVoice = intDriver->previewChannel >= 0 && intDriver->previewChannel < MAXTRACK &&
		ch == &intDriver->base.chan[intDriver->previewChannel];
	if (!previewVoice && !intDriver->base.Active[ch->TrackID])
		return 0;
	
	if (intDriver->base.curMusic != NULL && !previewVoice)
		temp = (temp * intDriver->base.curMusic->header->chanVol[ch->TrackID]) / MAX_VOLUME;
	else if (intDriver->base.curMusic == NULL)
		temp = 256*256;
...
void IntNoteOff(MADChannel *curVoice, MADDriverRec *intDriver)
{
	...
	if (intDriver->DriverSettings.driverMode == MIDISoundDriver ||
		MADMIDIPlaybackOutputIsEnabled()) {
		if (intDriver->NoteOld[curVoice->ID] != -1)
			NoteOff(intDriver->MIDIChannelOld[curVoice->ID], intDriver->NoteOld[curVoice->ID],
				intDriver->VelocityOld[curVoice->ID], intDriver);
		intDriver->NoteOld[curVoice->ID] = -1;
		intDriver->MIDIChannelOld[curVoice->ID] = -1;
	}
}
...
void NoteAnalyse(MADDriverRec *intDriver)
{
		...
		switch (intDriver->DriverSettings.outPutBits)
		{
			...
			case 16:
				...
				for (i = 0; i < MAXCHANEFFECT; i++) {
					if (intDriver->EffectBufferID[i] != -1) {
						/*
						 * Advance the per-track delay line in the same direction
						 * as the master buffer. The reversed copy retained the
						 * already-rendered block and accumulated it again on the
						 * next callback, producing a feedback-like buildup when
						 * the result was subsequently sent through global FX.
						 */
						memmove(intDriver->DASCEffectBuffer[i],
							intDriver->DASCEffectBuffer[i] + intDriver->ASCBUFFER*2,
							realMDelay*8L);
	...
	if (intDriver->base.Equalizer) {	// Apply Fourier Transform
		MADProcessOutputEqualizer(intDriver, intDriver->IntDataPtr,
			(size_t)intDriver->ASCBUFFERReal);
	}
...
void NoteOff(short midiChannel, short oldN, short oldV, MADDriverRec *intDriver)
{
	(void)intDriver;
	if (midiChannel < 0 || midiChannel > 15 || oldN < 0) return;
	int note = oldN + 12;
	if (note < 0 || note > 127) return;
	int velocity = oldV <= 0 ? 0 : (oldV * 127 + 32) / 64;
	if (velocity > 127) velocity = 127;
	MADMIDISendNoteOff((uint8_t)midiChannel, (uint8_t)note, (uint8_t)velocity);
}

void AllNoteOff(MADDriverRec *intDriver)
{
	if (intDriver == NULL) return;
	for (short i = 0; i < MAXTRACK; i++) {
		if (intDriver->NoteOld[i] != -1) {
			NoteOff(intDriver->MIDIChannelOld[i], intDriver->NoteOld[i],
				intDriver->VelocityOld[i], intDriver);
			intDriver->NoteOld[i] = -1;
			intDriver->MIDIChannelOld[i] = -1;
		}
	}
...
{
	if (curVoice == NULL || intDriver == NULL || curN < 0) return;
	int note = curN + 12;
	if (note < 0 || note > 127) return;
	uint8_t midiChannel = MADMIDIRoutedChannel(curVoice->TrackID, channel);
	int velocity = curVoice->vol <= 0 ? 1 : (curVoice->vol * 127 + 32) / 64;
	if (velocity > 127) velocity = 127;
	MADMIDISendProgramIfNeeded(midiChannel, (uint8_t)(channel & 0x7F));
	MADMIDISendNoteOn(midiChannel, (uint8_t)note, (uint8_t)velocity);
	intDriver->MIDIChannelOld[curVoice->ID] = midiChannel;
}

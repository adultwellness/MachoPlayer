...
static bool CAHasAudiblePreview(const MADDriverRec *driver)
{
	int channels = driver->DriverSettings.outPutMode == DeluxeStereoOutPut
		? driver->MultiChanNo : driver->DriverSettings.numChn;
	if (channels < driver->DriverSettings.numChn)
		channels = driver->DriverSettings.numChn;
	if (channels > MAXTRACK) channels = MAXTRACK;
	for (int channel = 0; channel < channels; channel++) {
		const MADChannel *voice = &driver->base.chan[channel];
		if (voice->samplePtr != NULL && (voice->curPtr < voice->maxPtr || voice->loopSize > 0)) return true;
	}
	return false;
}
								...
								AudioBufferList				*ioData)
{
	...
	// CABuffer can span several smaller Core Audio callbacks. Clearing it here
	// while transport is stopped destroys the unconsumed part of a sample
	// preview and produces alternating audio/silence. Refill it only after the
	// cached buffer has been consumed; the refill path below supplies silence
	// when neither transport nor an audition voice is active.
	...
	for (i = 0; i < ioData->mNumberBuffers; i++) {
		...
		while (remaining > 0) {
			if (theRec->CABufOff >= theRec->BufSize) {
				bool rendered = theRec->base.Reading
					? MADDirectSave(theRec->CABuffer, NULL, theRec)
					: (CAHasAudiblePreview(theRec) ? MADDirectSaveAlways(theRec->CABuffer, NULL, theRec) : false);
				if (!rendered) {

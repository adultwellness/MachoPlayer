...
MADErr MADCreateDriver(MADDriverSettings *DriverInitParam, MADLibrary *lib, MADDriverRec** returnDriver)
{
	...
	*returnDriver = NULL;
	...
	MDriver->previewChannel		= -1;
	MDriver->previewChannelBaseCount = -1;
	...
	// Core Audio is already running here, but no music or preview voice can be
	// rendered before the driver is returned. Install the built-in effect host
	// before the application can attach and start a song.
	MADInitializeBuiltinEffects(MDriver);
...
MADErr MADDisposeDriver(MADDriverRec* MDriver)
{
	...
	MADDisposeDriverBuffer(MDriver);
	MADDisposeBuiltinEffects(MDriver);
...
MADErr MADPlaySoundData(MADDriverRec *MDriver, const char *soundPtr, size_t size, int channel, MADByte note, int amplitude, size_t loopBeg, size_t loopSize, unsigned int rate, bool stereo)
{
	...
	if (MDriver == NULL)
		return MADParametersErr;
	bool reservedPreview = channel == MDriver->previewChannel &&
		channel >= 0 && channel < MDriver->MultiChanNo;
	if (channel < 0 || channel >= MAXTRACK ||
		(channel >= MDriver->DriverSettings.numChn && !reservedPreview))
		return MADParametersErr;
	
	// Pattern/sample mutations still use the historical whole-song guard. The
	// reserved Piano voice is outside the pattern, so engaging that guard here
	// would unnecessarily replace an entire Core Audio buffer with silence.
	if (MDriver->base.curMusic != NULL && !reservedPreview)

...
struct MADDriverRec {
	...
	double			*Filter, *fData;
	void			*outputEqualizerState;
	int				MDelay;
	int				RDelay;
	void			*ReverbPtr;
	short			PatDelay;
	short			lastChannelUsed[MAXTRACK];
	// Auxiliary voice reserved by the Piano for non-destructive auditioning.
	// It is mixed normally, but pattern playback must never allocate it.
	short			previewChannel;
	// Voice count to restore when the auxiliary Piano voice is released.
	short			previewChannelBaseCount;
